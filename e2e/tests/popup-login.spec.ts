...
test.describe('Popup Login Flow', () => {
  ...
  test('should complete authentication in popup and update parent', async () => {
    ...
    const emailLoginButton = popup.locator('button:has-text("Log in with Email"), a:has-text("Log in with Email")')
    if (await emailLoginButton.isVisible({ timeout: 3000 }).catch(() => false)) {
      await emailLoginButton.click()
    }

    // Fill credentials
    await popup.fill('input[name="login"], input[type="email"], input#login', 'admin@example.com')
    await popup.fill('input[name="password"], input[type="password"]', 'password')
  ...
  test('should prevent multiple simultaneous popup attempts', async () => {
    ...
    await page
      .waitForFunction(() => sessionStorage.getItem('popup_loginInProgress') === 'true', { timeout: 2000 })
      .catch(() => {})
  ...
  test('should store loginMethod as popup', async () => {
    ...
    await page.waitForFunction(() => sessionStorage.getItem('popup_loginMethod') !== null, { timeout: 2000 })
...
test.describe('Popup Window Positioning', () => {
  test('should open popup with reasonable dimensions', async ({ browser }) => {
    ...
    popupFeatures = await page.evaluate(() => (window as any).__popupFeatures || '')
...
test.describe('Popup Error Handling', () => {
  test('should handle popup closed by user before auth completes', async ({ browser }) => {
    const context = await browser.newContext()
    const page = await context.newPage()
    ...
    await page
      .waitForFunction(
        () => {
          ...
          return inProgress === 'false' || !sessionStorage.getItem('popup_token')
        },
        { timeout: 3000 }
      )
      .catch(() => {})
    ...
    const token = await page.evaluate(() => sessionStorage.getItem('popup_token'))
  ...
  test('should handle auth error in popup', async ({ browser }) => {
    ...
    await popup.fill('input[name="login"], input[type="email"], input#login', 'wrong@example.com')
    await popup.fill('input[name="password"], input[type="password"]', 'wrongpassword')
    await popup.click('button[type="submit"], input[type="submit"]')
