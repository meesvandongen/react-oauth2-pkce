...
test.describe("Token Endpoint Error Handling", () => {
	...
	test("should handle 500 error during token refresh", async ({ page }) => {
		...
		await expect(async () => {
			...
			const refreshAttempted = await page.evaluate(
				() => sessionStorage.getItem("network_refreshInProgress") !== null,
			);
	...
	test("should handle 401 unauthorized during token exchange", async ({
		page,
	}) => {
	...
	test("should handle timeout during token exchange", async ({ page }) => {
		...
		await expect(async () => {
			const loginInProgress = await page.evaluate(() =>
				sessionStorage.getItem("network_loginInProgress"),
			);
...
test.describe("Malformed Response Handling", () => {
	...
	test("should handle response missing access_token", async ({ page }) => {
		...
		const isAuthenticated = await page.evaluate(
			() => !!sessionStorage.getItem("network_token"),
		);
...
test.describe("Network Retry and Recovery", () => {
	...
	test("should handle intermittent network failures during refresh", async ({
		page,
	}) => {
		await page.goto("/network");
...
test.describe("Authorization Endpoint Errors", () => {
	test("should handle authorization endpoint returning error", async ({
		page,
	}) => {
		await page.goto("/network");
		...
		await page.goto(
			"/network?error=access_denied&error_description=User%20denied%20access",
		);
