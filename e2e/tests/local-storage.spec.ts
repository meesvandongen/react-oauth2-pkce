...
test.describe('Local Storage Feature', () => {
  ...
  test('should display localStorage contents for verification', async ({ page }) => {
    await page.click('[data-testid="login-button"]')
    await performDexLogin(page)
    await waitForAuthenticated(page)

    // The page shows localStorage contents
    const storageContents = await page.locator('[data-testid="storage-contents"]').textContent()
    expect(storageContents).toContain('localstorage_token')
  })

  test('should persist across browser sessions (simulated)', async ({ page, context }) => {
    await page.click('[data-testid="login-button"]')
    await performDexLogin(page)
    await waitForAuthenticated(page)
  ...
  test('should clear localStorage on logout', async ({ page }) => {
    ...
    // Verify token exists
    let localToken = await page.evaluate(() => localStorage.getItem('localstorage_token'))
    expect(localToken).toBeTruthy()
    ...
    localToken = await page.evaluate(() => localStorage.getItem('localstorage_token'))
    expect(localToken === null || localToken === '""' || localToken === '').toBeTruthy()
  })
})
