...
test.describe("Auto Login Feature", () => {
	...
	test("should automatically redirect to login when autoLogin is enabled", async ({
		page,
	}) => {
	...
	test("should NOT show state mismatch error during auto-login flow", async ({
		page,
	}) => {
	...
	test("should stay authenticated after auto-login on page reload", async ({
		page,
	}) => {
		await page.goto("/autologin");
	...
	test("should show state mismatch error when state is tampered during OAuth flow", async ({
		page,
	}) => {
