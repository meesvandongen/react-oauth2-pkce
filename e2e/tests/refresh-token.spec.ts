...
test.describe('Refresh Token Handling', () => {
  ...
  test('should receive refresh token with offline_access scope', async ({ page }) => {
  ...
  test('should configure renewable refresh token strategy', async ({ page }) => {
...
test.describe('Login In Progress State', () => {
  ...
  test('should clear login in progress after successful login', async ({ page }) => {
    await page.click('[data-testid="login-button"]')
    await performDexLogin(page)
    await waitForAuthenticated(page)
