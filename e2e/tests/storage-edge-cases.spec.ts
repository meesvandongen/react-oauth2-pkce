...
test.describe('Storage Cleared During Auth Flow', () => {
  test('should handle storage cleared after redirect to IdP', async ({ page }) => {
    await page.goto('/basic')
    await clearBrowserStorage(page)
    await page.reload()
  ...
  test('should handle storage cleared before token exchange', async ({ page }) => {
    await page.goto('/basic')
    await clearBrowserStorage(page)
    await page.reload()
...
test.describe('SessionStorage Tab Isolation', () => {
  ...
  test('should have isolated sessionStorage per tab', async () => {
    ...
    const value = await page2.evaluate(() => sessionStorage.getItem('test_key'))
...
test.describe('Storage Key Prefix Collisions', () => {
  test('should prevent collisions with different prefixes', async ({ page }) => {
    await page.goto('/basic')
    await clearBrowserStorage(page)
    await page.reload()
    ...
    const customToken = await page.evaluate(() => sessionStorage.getItem('customstate_token'))
...
test.describe('Storage Persistence', () => {
  test('should persist token in sessionStorage across page reload', async ({ page }) => {
    await page.goto('/basic')
    await clearBrowserStorage(page)
    await page.reload()
  ...
  test('should persist token in localStorage across browser context restart', async ({ browser }) => {
    // First session
    ...
    // Get token
    const _token = await page1.evaluate(() => localStorage.getItem('localstorage_token'))
...
test.describe('Storage Event Listeners', () => {
  ...
  test('should respond to storage changes from other tabs', async () => {
    ...
    await page2
      .waitForFunction(() => (window as any).__storageEvents && (window as any).__storageEvents.length > 0, {
        timeout: 2000,
      })
      .catch(() => {})
...
test.describe('Storage JSON Serialization', () => {
  test('should correctly serialize and deserialize token data', async ({ page }) => {
    await page.goto('/basic')
    await clearBrowserStorage(page)
    await page.reload()
