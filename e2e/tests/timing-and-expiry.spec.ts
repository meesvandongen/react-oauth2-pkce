...
test.describe('Token Timing and Automatic Refresh', () => {
  ...
  test('should trigger automatic refresh when token expires', async ({ page }) => {
    ...
    await expect(async () => {
      const newToken = await page.evaluate(() => sessionStorage.getItem('timing_token'))
  ...
  test('should handle very short token lifetimes (< 30 seconds)', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="config-token-expires"]')).toContainText('5 seconds')
  })
})

test.describe('Renewable vs Absolute Expiry Strategy', () => {
  test('renewable strategy should extend refresh token expiry on use', async ({ page }) => {
    await page.goto('/timing')
    await clearBrowserStorage(page)
    await page.reload()
    ...
    await expect(page.locator('[data-testid="config-strategy"]')).toContainText('renewable')
  })

  test('absolute strategy should NOT extend refresh token expiry on use', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="config-strategy"]')).toContainText('absolute')
  })

  test('absolute expiry should trigger callback when refresh token lifetime ends', async ({ page }) => {
    await page.goto('/absolute-expiry')
    await clearBrowserStorage(page)
...
test.describe('Refresh Token Expiry Callback', () => {
  ...
  test('onRefreshTokenExpire callback should provide logIn function', async ({ page }) => {
    await page.click('[data-testid="login-button"]')
    await performDexLogin(page)
    await waitForAuthenticated(page)
    ...
    await expect(async () => {
      const callbackInvoked = await page.evaluate(() => window.timingRefreshExpiredCallbackInvoked)
      const hasNoToken = await page.evaluate(() => !sessionStorage.getItem('timing_token'))
    ...
    const callbackStatus = await page.locator('[data-testid="callback-status"]').textContent()
  ...
  test('should be able to re-login through callback event', async ({ page }) => {
    await page.click('[data-testid="login-button"]')
    await performDexLogin(page)
    await waitForAuthenticated(page)
...
test.describe('Token Refresh Interval', () => {
  test('should check for token expiry periodically', async ({ page }) => {
    ...
    await expect(async () => {
      const token = await page.evaluate(() => sessionStorage.getItem('timing_token'))
  ...
  test('should handle multiple refresh cycles', async ({ page }) => {
    ...
    const _initialToken = await page.evaluate(() => sessionStorage.getItem('timing_token'))
    ...
    // Get refresh count
    const refreshCount = await page.evaluate(() => window.timingTokenRefreshCount)
...
test.describe('Token Expiry Edge Cases', () => {
  test('should handle page reload during token refresh', async ({ page }) => {
    ...
    const hasToken = await page.evaluate(() => !!sessionStorage.getItem('timing_token'))
  ...
  test('should handle missing refreshToken gracefully', async ({ page }) => {
    ...
    await expect(async () => {
      ...
      const tokenExpired = await page.evaluate(
        () => !sessionStorage.getItem('timing_token') || sessionStorage.getItem('timing_tokenExpire') === '0'
      )
  ...
  test('should handle zero/negative expiry times', async ({ page }) => {
    ...
    await expect(async () => {
      const refreshInProgress = await page.evaluate(() => sessionStorage.getItem('timing_refreshInProgress'))
      const callbackInvoked = await page.evaluate(() => window.timingRefreshExpiredCallbackInvoked)
