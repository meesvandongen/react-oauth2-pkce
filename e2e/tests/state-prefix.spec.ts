...
test.describe("State Validation", () => {
	...
	test("should handle empty state from OAuth server gracefully", async ({
		page,
	}) => {
	...
	test("should show state mismatch error when actual tampering occurs", async ({
		page,
	}) => {
		...
		const hasStateMismatchError = consoleErrors.some((e) =>
			e.includes("state"),
		);
