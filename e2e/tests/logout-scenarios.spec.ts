...
test.describe("Logout with LogoutEndpoint", () => {
	test("should redirect to logoutEndpoint when configured", async ({
		page,
	}) => {
		...
		await expect(
			page.locator('[data-testid="config-logout-endpoint"]'),
		).toContainText("dex/logout");
	...
	test("should not redirect when logoutEndpoint is not configured", async ({
		page,
	}) => {
		await page.goto("/basic");
...
test.describe("Logout with State Parameter", () => {
	test("should include state in logout request", async ({ page }) => {
		...
		const requestPromise = page
			.waitForRequest((request) => request.url().includes("logout"), {
				timeout: 2000,
			})
			.catch(() => null);
		...
		await Promise.race([
			requestPromise,
			page.waitForFunction(() => !sessionStorage.getItem("logouttest_token"), {
				timeout: 2000,
			}),
		]).catch(() => {});
...
test.describe("Logout with Full Parameters", () => {
	...
	test("should include id_token_hint when available", async ({ page }) => {
		...
		await expect(
			page.locator('[data-testid="id-token-available"]'),
		).toContainText("yes");
...
test.describe("Logout with Expired Tokens", () => {
	test("should logout successfully even when token is expired", async ({
		page,
	}) => {
		await page.goto("/basic");
...
test.describe("Logout Redirect", () => {
	test("should include post_logout_redirect_uri in request", async ({
		page,
	}) => {
		await page.goto("/logout-test");
		...
		await expect(
			page.locator('[data-testid="config-logout-redirect"]'),
		).toContainText("logout-test");
...
test.describe("Logout Error Handling", () => {
	test("should handle logout endpoint errors gracefully", async ({ page }) => {
		...
		const token = await page.evaluate(() =>
			sessionStorage.getItem("logouttest_token"),
		);
		expect(token).toBeFalsy();
	});
...
test.describe("Extra Logout Parameters", () => {
	test("should include extraLogoutParameters in request", async ({ page }) => {
		...
		await expect(
			page.locator('[data-testid="config-extra-params"]'),
		).toContainText("custom_logout_param");
	...
	test("should merge additionalParameters with extraLogoutParameters", async ({
		page,
	}) => {
		await page.goto("/logout-test");
...
test.describe("Logout Standard Parameters", () => {
	...
	test("should use refresh_token for logout when available", async ({
		page,
	}) => {
		await page.goto("/logout-test");
