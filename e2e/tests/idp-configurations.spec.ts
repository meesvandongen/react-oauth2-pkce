...
test.describe("Different Refresh Token Expiry Field Names", () => {
	...
	test("should handle Azure AD style refresh_token_expires_in", async ({
		page,
	}) => {
		// Intercept token endpoint
		await page.route("**/dex/token", async (route, request) => {
...
test.describe("Expiry Time Edge Cases", () => {
	...
	test("should handle string expires_in (some IdPs return strings)", async ({
		page,
	}) => {
		await page.route("**/dex/token", async (route, request) => {
			const postData = request.postData() || "";
...
test.describe("Opaque Token and decodeToken: false", () => {
	test("should handle opaque tokens with decodeToken: false", async ({
		page,
	}) => {
		...
		const tokenDataText = await page
		...
		await expect(
			page.locator('[data-testid="config-decode-token"]'),
		).toContainText("false");
	});
...
test.describe("clearURL: false Configuration", () => {
	test("should keep URL parameters when clearURL is false", async ({
		page,
	}) => {
		...
		await expect(
			page.locator('[data-testid="config-clear-url"]'),
		).toContainText("false");
	});
});
...
test.describe("IdP Response with Invalid JWT", () => {
	test("should handle invalid JWT gracefully when decodeToken is true", async ({
		page,
	}) => {
		await page.goto("/network");
