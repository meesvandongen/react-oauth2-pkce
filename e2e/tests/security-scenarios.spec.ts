...
test.describe('CSRF Protection via State Parameter', () => {
  ...
  test('should reject mismatched state parameter', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      sessionStorage.setItem('basic_PKCE_code_verifier', 'test-verifier-abc123')
    })
  ...
  test('should handle null state in storage vs empty state from IdP', async ({ page }) => {
  ...
  test('should handle missing state from IdP when none was sent', async ({ page }) => {
...
test.describe('Code Verifier Security', () => {
  ...
  test('should fail if code_verifier is tampered with', async ({ page }) => {
    ...
    await page.evaluate(() => {
      sessionStorage.setItem('basic_PKCE_code_verifier', 'tampered-verifier-xyz')
    })
  ...
  test('should clear code_verifier after successful token exchange', async ({ page }) => {
    await page.click('[data-testid="login-button"]')
    await performDexLogin(page)
    await waitForAuthenticated(page)
...
test.describe('Replay Attack Prevention', () => {
  ...
  test('should prevent auth code reuse', async ({ page }) => {
    ...
    const _originalToken = await page.evaluate(() => sessionStorage.getItem('basic_token'))
...
test.describe('Storage Key Prefix Isolation', () => {
  test('should isolate auth state between different providers', async ({ page }) => {
    // Login to basic auth
    await page.goto('/basic')
    await clearBrowserStorage(page)
    await page.reload()
    ...
    const basicToken = await page.evaluate(() => sessionStorage.getItem('basic_token'))
    expect(basicToken).toBeTruthy()

    // Check that localStorage (different app) doesn't have this
    const localToken = await page.evaluate(() => localStorage.getItem('basic_token'))
    ...
    const wrongToken = await page.evaluate(() => localStorage.getItem('basic_token'))
    expect(wrongToken).toBeNull()
  })

  test('should prevent state collision with different prefixes', async ({ page }) => {
...
test.describe('Authorization Code Injection', () => {
  ...
  test('should not process code without loginInProgress flag', async ({ page }) => {
    ...
    const hasToken = await page.evaluate(() => !!sessionStorage.getItem('basic_token'))
...
test.describe('Token Storage Security', () => {
  test('should store tokens only in configured storage type', async ({ page }) => {
    ...
    const sessionToken = await page.evaluate(() => sessionStorage.getItem('basic_token'))
    const localToken = await page.evaluate(() => localStorage.getItem('basic_token'))
  ...
  test('should store tokens in localStorage when configured', async ({ page }) => {
    await page.goto('/localstorage')
    await clearBrowserStorage(page)
    await page.reload()
    ...
    const localToken = await page.evaluate(() => localStorage.getItem('localstorage_token'))
...
test.describe('XSS Prevention', () => {
  test('should not execute script in error messages', async ({ page }) => {
    ...
    await page.goto(`/basic?error=test&error_description=${encodeURIComponent(xssPayload)}`)
...
test.describe('PKCE Code Challenge', () => {
  test('should generate unique code_verifier for each login', async ({ page }) => {
    await page.goto('/basic')
    await clearBrowserStorage(page)
    await page.reload()
    ...
    await page.route('**/dex/auth**', async (route, request) => {
      firstVerifier = new URL(request.url()).searchParams.get('code_challenge') || ''
    ...
    let secondVerifier = ''
    await page.route('**/dex/auth**', async (route, request) => {
      secondVerifier = new URL(request.url()).searchParams.get('code_challenge') || ''
      await route.continue()
    })
  ...
  test('should use SHA256 for code_challenge_method', async ({ page }) => {
    ...
    let challengeMethod = ''
    await page.route('**/dex/auth**', async (route, request) => {
      challengeMethod = new URL(request.url()).searchParams.get('code_challenge_method') || ''
      await route.continue()
    })
