...
test.describe('Pre/Post Login Hooks', () => {
  ...
  test('should call preLogin before redirecting to auth provider', async ({ page }) => {
    // Clear callback status first
    await page.click('[data-testid="clear-status"]')

    // Verify initial state
    await expect(page.locator('[data-testid="prelogin-status"]')).toHaveText('not-called')
    ...
    await expect(page.locator('[data-testid="prelogin-status"]')).toHaveText('called')
  })

  test('should call postLogin after successful token exchange', async ({ page }) => {
    // Clear callback status first
    await page.click('[data-testid="clear-status"]')
    ...
    await page.click('[data-testid="refresh-status"]')
    await expect(page.locator('[data-testid="postlogin-status"]')).toHaveText('called')
  })

  test('should call both preLogin and postLogin in correct order', async ({ page }) => {
    await page.click('[data-testid="clear-status"]')
    ...
    const preTimestamp = await page.evaluate(() => sessionStorage.getItem('preLoginTimestamp'))
    const postTimestamp = await page.evaluate(() => sessionStorage.getItem('postLoginTimestamp'))
