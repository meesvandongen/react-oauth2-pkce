...
test.describe('Basic Authentication Flow', () => {
  ...
  test('should perform OAuth2 PKCE login flow with redirect', async ({ page }) => {
    // Click login button
  ...
  test('should perform OAuth2 PKCE login flow with replace', async ({ page }) => {
  ...
  test('should decode access token when decodeToken is true', async ({ page }) => {
    await page.click('[data-testid="login-button"]')
    await performDexLogin(page)
    await waitForAuthenticated(page)

    // Check that token data is present and contains expected fields
    const tokenData = await page.locator('[data-testid="token-data"]').textContent()
  ...
  test('should use session storage by default', async ({ page }) => {
    ...
    const sessionToken = await page.evaluate(() => {
      return sessionStorage.getItem('basic_token')
    })
    expect(sessionToken).toBeTruthy()

    // Local storage should not have the token
