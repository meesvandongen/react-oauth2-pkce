...
test("preserves URL parameters when clearURL is false", async ({ page }) => {
	...
	// Use button that passes an app-defined round-trip state value
...
test("does not include scope in refresh request when refreshWithScope is false", async ({
	...
	oauth.accessTokenLifetimeSeconds = 20; // 20 seconds
...
test("uses manual expiry overrides", async ({ page, oauth }) => {
	oauth.accessTokenLifetimeSeconds = 20;
...
test("respects absolute refreshTokenExpiryStrategy", async ({
	...
	oauth.refreshTokenLifetimeSeconds = 100;
	oauth.accessTokenLifetimeSeconds = 40;
